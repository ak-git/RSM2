...
smthA_RHO_S1 <- smth(out$A_RHO_S1, window = window, method = "gaussian")
lines(out$TIME, smthA_RHO_S1, col = "black", lty = "solid", lwd = 2)
...
smthA_RHO_S1_DIFF <- smth(out$dA_RHO_S1, window = window, method = "gaussian")
lines(out$TIME, smthA_RHO_S1_DIFF, col = "black", lty = "solid", lwd = 2)

plot(out$TIME, out$A_RHO_S2, lty = 'solid', type = 'l', xlab = 'Time, s', ylab = expression(bold(rho ~ " 30 x 18 mm")), col = 'red')
smthA_RHO_S2 <- smth(out$A_RHO_S2, window = window, method = "gaussian")
lines(out$TIME, smthA_RHO_S2, col = "black", lty = "solid", lwd = 2)
...
smthA_RHO_S2_DIFF <- smth(out$dA_RHO_S2, window = window, method = "gaussian")
lines(out$TIME, smthA_RHO_S2_DIFF, col = "black", lty = "solid", lwd = 2)
