...
lines(outR$TIME, outR$R1, type = 'b', lty = 'blank', col = col[1])

plot(df$TIME, df$R2, type = 'l', xlab = xlab, ylab = substitute(bold(R[s ~ x ~ L ~ mm] ~ ~Omega), list(s = mmBase * 5, L = mmBase * 3)))
lines(outR$TIME, outR$R2, type = 'b', lty = 'blank', col = col[2])
...
outA <- sapply(1:(length(outPosition$TIME)),
               function(x) {
                 R1 <- max(mean(c(outR$R1[x - 1], outR$R1[x + 1])), outR$R1[x])
                 R2 <- max(mean(c(outR$R2[x - 1], outR$R2[x + 1])), outR$R2[x])
                 A1 <- layer1Inverse(mmBase, mmBase * 3.0, R1)
                 A2 <- layer1Inverse(mmBase * 5.0, mmBase * 3.0, R2)

                 DR1 <- mean(
                   c(
                     (outR$R1[x] - outR$R1[x - 1]) / ((outPosition$POSITION[x] - outPosition$POSITION[x - 1]) / (mmBase * 3)),
                     (outR$R1[x + 1] - outR$R1[x]) / ((outPosition$POSITION[x + 1] - outPosition$POSITION[x]) / (mmBase * 3))
                   )
                 )
                 DR2 <- mean(
                   c(
                     (outR$R2[x] - outR$R2[x - 1]) / ((outPosition$POSITION[x] - outPosition$POSITION[x - 1]) / (mmBase * 3)),
                     (outR$R2[x + 1] - outR$R2[x]) / ((outPosition$POSITION[x + 1] - outPosition$POSITION[x]) / (mmBase * 3))
                   )
                 )
