...
col <- hue_pal()(3)
lwd <- 3
plot(df$TIME, df$POSITION, type = 'l', xlab = xlab, ylab = 'mm', col = col[1], lwd = lwd,
     main = 'Положение электродной системы по вертикали')
plot(df$TIME, df$R1, type = 'l', xlab = xlab, ylab = expression(Omega), col = col[2], lwd = lwd,
     main = substitute(bold(R[s ~ x ~ L ~ mm]), list(s = mmBase, L = mmBase * 3)))
plot(df$TIME, df$R2, type = 'l', xlab = xlab, ylab = expression(Omega), col = col[3], lwd = lwd,
